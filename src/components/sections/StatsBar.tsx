        ...
        <dl className="flex items-center w-[1108px]">
          {STATS.map(({ value, label }, index) => (
            <div key={label} className="flex items-center flex-1">
              <div className="flex-1 flex flex-col items-center gap-[6px] text-center">
                <dt className="order-2 font-sans text-[13px] text-muted">
                  {label}
                </dt>
                <dd className="order-1 font-heading font-bold text-[36px] leading-none text-primary">
                  {value}
                </dd>
              </div>
      ...
      <dl className="md:hidden grid grid-cols-2 divide-x divide-y divide-divider">
        {STATS.map(({ value, label }) => (
          <div key={label} className="flex flex-col items-center gap-[6px] py-6 px-4 text-center">
            <dt className="order-2 font-sans text-[12px] text-muted">
              {label}
            </dt>
