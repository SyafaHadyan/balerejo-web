...
function wisataPopupHTML(spot: WisataSpot): string {
  const googleMapsUrl = `https://www.google.com/maps/dir/?api=1&destination=${spot.lat},${spot.lng}`;
  /* Truncate description to ~100 chars for popup */
  const desc = spot.description.length > 100
    ? spot.description.slice(0, 100).trimEnd() + "…"
    : spot.description;
  return `
    <div style="display:flex;flex-direction:column;gap:6px;padding:14px;width:210px;font-family:sans-serif;box-sizing:border-box;">
      <p style="font-size:11px;font-weight:700;color:#37644d;margin:0;letter-spacing:1px;">${TYPE_LABEL[spot.type]}</p>
      <p style="font-family:'Playfair Display',Georgia,serif;font-weight:700;font-size:14px;color:#1D392B;line-height:1.3;margin:0;padding-right:22px;">${spot.name}</p>
      <p style="font-size:11px;color:#61665e;line-height:1.5;margin:0;">${desc}</p>
...
function MapController({
  ...
  }, [pin, product, wisataSpot, map, isLoaded, popup]);
...
function UserLocationMarker({ onLocated }: { onLocated: (coords: [number, number]) => void }) {
      ...
      <MarkerContent>
        <div style={{ position: "relative", width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{
            ...
            background: "rgba(66,133,244,0.25)", animation: "umkm-ping 1.6s ease-out infinite",
          }} />
          <div style={{
            width: 14, height: 14, borderRadius: "50%", background: "#4285F4",
            border: "2.5px solid white", boxShadow: "0 2px 6px rgba(0,0,0,0.3)", flexShrink: 0,
          }} />
        </div>
      </MarkerContent>
    </MapMarker>
...
export default function UmkmMap({
  ...
  return (
    ...
    >
      ...
      {/* Wisata markers — amber pin, distinct from UMKM */}
      {WISATA.map((spot) => {
        const id = `wisata:${spot.slug}`;
        const isSelected = id === selectedSlug;
        return (
          <MapMarker
            key={id}
            longitude={spot.lng}
          ...
          >
            <MarkerContent>
              ...
              >
                <path
                  d="M14 0C6.268 0 0 6.268 0 14c0 9.333 14 22 14 22S28 23.333 28 14C28 6.268 21.732 0 14 0z"
                  fill={isSelected ? "#c0392b" : "#7C5A2A"}
                />
                <circle cx="14" cy="14" r="5.5" fill={isSelected ? "#fff" : "#f5dfa0"} />
              </svg>
            </MarkerContent>
          </MapMarker>
        );
